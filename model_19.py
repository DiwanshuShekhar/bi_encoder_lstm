...
class BiEncoderModel(object):
    ...
    def inference(self, data):
        ...
        generated_response = tf.matmul(encoding_context, M)
        norm_vec = tf.norm(generated_response, axis=1)
        norm_vec = tf.reshape(norm_vec, [-1, 1])
        generated_response = tf.divide(generated_response, norm_vec)
        #generated_response = tf.expand_dims(generated_response, 2)
        print ("Shape of normalized gen res {}".format(generated_response.shape))
        #encoding_utterance = tf.expand_dims(encoding_utterance, 2)
        norm_vec = tf.norm(encoding_utterance, axis=1)
        norm_vec = tf.reshape(norm_vec, [-1, 1])
        encoding_utterance = tf.divide(encoding_utterance, norm_vec)
        print ("Shape of normalized enc utt {}".format(encoding_utterance.shape))
        # generated_response = tf.expand_dims(generated_response, 2)
